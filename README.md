...
| ---------------------------- | ------------------------------ | -------- | ---------------------- |
| `x86_64-pc-windows-msvc`     | `meta_mystia_network.dll`      | `.pdb`   | Windows 宿主           |
| `aarch64-pc-windows-msvc`    | `meta_mystia_network.dll`      | `.pdb`   | Windows 宿主           |
| `x86_64-pc-windows-gnu`      | `meta_mystia_network.dll`      | —        | macOS / Linux 交叉编译 |
| `aarch64-pc-windows-gnullvm` | `meta_mystia_network.dll`      | —        | macOS / Linux 交叉编译 |
...
- 非 Windows 宿主（macOS / Linux）：所有交叉编译目标（含 Windows）均使用 [cargo-zigbuild](https://github.com/rust-cross/cargo-zigbuild) + [Zig](https://ziglang.org/)，Windows x86_64 使用 GNU ABI（`x86_64-pc-windows-gnu`），aarch64 使用 gnullvm（`aarch64-pc-windows-gnullvm`）

```powershell
./build_all.ps1
```

该脚本为每个目标平台分别构建默认版本与 `logging` 版本（共 10 次）。在 Windows 宿主上，Windows 目标使用 `cargo build`（MSVC）；在 macOS / Linux 宿主上，所有目标（含 Windows GNU）均使用 `cargo zigbuild`。所有产物统一复制到 `target/output/` 目录并按 `{name}[-logging]-{target}.{ext}` 格式重命名。
