...
为 MetaMystia Mod 实现一个 P2P 网络库，编译为 cdylib（Windows `.dll` / macOS `.dylib`），通过 `extern "C"` FFI 供 C# IL2CPP 调用。
...
| `plan-FFIIntegration.prompt.md`    | callback.rs、ffi.rs、lib.rs、日志系统                            |
...
| `src/types.rs`          | 公共类型：`PeerId`, `PeerAddr`, `MessageTarget`, `Packet`, `PeerStatus` 等 |
| `src/protocol.rs`       | 协议常量（版本号、msg_type 定义、最大消息大小）+ InternalMessage 枚举      |
| `src/transport.rs`      | TCP 连接管理、粘包处理（`PacketCodec`）、自动重连、TCP Keep-alive          |
...
| `src/session_router.rs` | 消息路由（Broadcast/ToPeer/ToLeader）+ 中心化模式 + 发送队列管理           |
...
sequenceDiagram
    ...
    B->>A: HandshakeAck { peer_id, listen_port, success: true }
...
| **可测试** | 子系统（`MembershipManager`、`LeaderElection`）为纯状态管理器，不持有网络 IO 引用，可独立单元测试；`TransportManager::new()` 返回 `(Self, incoming_rx)` 分离 IO 与逻辑 |
...
| broadcast channel | 成员事件用 `tokio::sync::broadcast`         | 多订阅者；Lagged 时全量同步 |
...
| PeerListSync   | 握手后同步已知 peer 列表                     | 补充 mDNS、加速拓扑收敛 |
...
| Transport 解耦           | `new()` 返回 `(Self, incoming_rx)`                | IO 与逻辑分离，可测试         |
...
5. **多进程集成测试**
   - 在本机启动 3 个进程（不同 peer_id，相同 session_id）
